...
const NIEUWS = [
  { titel: 'Goudprijs bereikt nieuw record door onzekerheid op markten', bron: 'Financial Times', tijd: '2 uur geleden', tag: 'Goud', kleur: '#f0b429', inhoud: 'De goudprijs heeft vandaag een nieuw record bereikt van meer dan $3.200 per troy ounce. Analisten wijzen op toenemende geopolitieke spanningen en inflatiezorgen als voornaamste oorzaken. Centrale banken blijven massaal goud aankopen als veilige haven.' },
  { titel: 'Centrale banken verhogen goudreserves tot hoogste niveau ooit', bron: 'Bloomberg', tijd: '4 uur geleden', tag: 'Goud', kleur: '#f0b429', inhoud: 'Centrale banken wereldwijd hebben hun goudreserves verhoogd tot het hoogste niveau in 50 jaar. China en India leiden de lijst van kopers met respectievelijk 150 en 80 ton aangekocht dit kwartaal.' },
  { titel: 'Zilverprijs stijgt door groeiende vraag uit zonne-energiesector', bron: 'Reuters', tijd: '5 uur geleden', tag: 'Zilver', kleur: '#a8b2bf', inhoud: 'De vraag naar zilver voor zonnepanelen groeit explosief. Experts verwachten dat de zilverprijs de komende jaren sterk zal stijgen door de energietransitie.' },
  { titel: 'Kopermarkt onder druk door vertraging Chinese industrie', bron: 'Wall Street Journal', tijd: '6 uur geleden', tag: 'Koper', kleur: '#e07b54', inhoud: 'De koperprijs staat onder druk na tegenvallende industriecijfers uit China. De vraag naar koper daalde met 3% in het laatste kwartaal.' },
  { titel: 'ECB houdt rente stabiel — euro versterkt tegenover dollar', bron: 'De Tijd', tijd: '7 uur geleden', tag: 'Economie', kleur: '#58a6ff', inhoud: 'De Europese Centrale Bank heeft besloten de rente ongewijzigd te laten op 3.5%. De euro reageerde positief en steeg 0.8% tegenover de dollar.' },
  { titel: 'BEL 20 sluit week positief af ondanks internationale onrust', bron: 'De Standaard', tijd: '8 uur geleden', tag: 'Beurs', kleur: '#3fb950', inhoud: 'De Belgische beurs sloot de week positief af met een winst van 1.2%. AB InBev en UCB waren de grote winnaars.' },
  { titel: 'Bitcoin doorbreekt $65.000 grens na ETF goedkeuring', bron: 'CoinDesk', tijd: '3 uur geleden', tag: 'Crypto', kleur: '#f7931a', inhoud: 'Bitcoin heeft de $65.000 grens doorbroken na positief nieuws over ETF goedkeuringen. Institutionele beleggers stromen massaal de cryptomarkt in.' },
  { titel: 'Ethereum upgrade maakt netwerk 30% sneller', bron: 'The Block', tijd: '6 uur geleden', tag: 'Crypto', kleur: '#627eea', inhoud: 'De laatste Ethereum upgrade heeft het netwerk significant sneller gemaakt. Transactiekosten zijn gedaald met 40% en de verwerkingssnelheid is toegenomen.' },
];
...
const MODULES = [
  { id: 1, titel: 'Wat is traden?', icoon: '📚', duur: '5 min', niveau: 'Beginner', kleur: '#3fb950', inhoud: `Traden betekent het kopen en verkopen van financiële activa zoals aandelen, metalen of crypto met als doel winst te maken.\n\nEr zijn verschillende soorten traders:\n• Day traders: kopen en verkopen binnen één dag\n• Swing traders: houden posities enkele dagen tot weken aan\n• Long-term investors: houden posities maanden tot jaren aan\n\nHet belangrijkste principe van traden is "koop laag, verkoop hoog".` },
  { id: 2, titel: 'Hoe werkt een portfolio?', icoon: '📊', duur: '7 min', niveau: 'Beginner', kleur: '#58a6ff', inhoud: `Een portfolio is een verzameling van alle investeringen die je bezit.\n\nDiversificatie is het sleutelwoord: door te investeren in verschillende activa verminder je het risico.\n\nStel je voor: als je alles in Bitcoin investeert en Bitcoin daalt 50%, verlies je de helft. Maar als je ook goud, aandelen en zilver hebt, compenseren die andere investeringen het verlies gedeeltelijk.` },
  { id: 3, titel: 'Technische analyse basics', icoon: '📈', duur: '10 min', niveau: 'Gemiddeld', kleur: '#f0b429', inhoud: `Technische analyse is het bestuderen van prijsgrafieken om toekomstige prijsbewegingen te voorspellen.\n\nBelangrijke concepten:\n• Support & Resistance: prijsniveaus waar een actief stopt\n• Trend: de algemene richting van de prijs\n• Volume: hoeveel er verhandeld wordt\n• Moving Average: het gemiddelde van de prijs over een periode` },
  { id: 4, titel: 'Risicobeheer & Stop-Loss', icoon: '🛡️', duur: '8 min', niveau: 'Gemiddeld', kleur: '#e07b54', inhoud: `Risicobeheer is het belangrijkste aspect van succesvol traden.\n\nDe 1% regel: risikeer nooit meer dan 1-2% van je totale portfolio op één trade.\n\nStop-Loss: een automatische verkooporder die je verlies beperkt. Stel je koopt Bitcoin op €60.000 en zet een stop-loss op €57.000 — als de prijs daalt naar €57.000 wordt er automatisch verkocht.` },
  { id: 5, titel: 'Fundamentele analyse', icoon: '🔍', duur: '12 min', niveau: 'Gevorderd', kleur: '#a8b2bf', inhoud: `Fundamentele analyse kijkt naar de onderliggende waarde van een actief.\n\nVoor aandelen kijk je naar:\n• Winst per aandeel\n• Koers/winst verhouding\n• Omzetgroei\n\nVoor crypto:\n• Adoptie en gebruikersgroei\n• Technologie\n• Tokenomics` },
  { id: 6, titel: 'Psychologie van traden', icoon: '🧠', duur: '6 min', niveau: 'Alle niveaus', kleur: '#9945ff', inhoud: `De grootste vijand van een trader is zichzelf.\n\nFOMO: je koopt omdat de prijs snel stijgt en je bang bent de boot te missen.\n\nGouden regels:\n• Maak een plan voor je tradet\n• Accepteer verliezen\n• Trade nooit met geld dat je niet kan missen\n• Houd een trading journal bij` },
];
...
function Navbar({ valuta, setValuta }: any) {
  return (
...
function ArtikelDetail({ artikel, onTerug }: any) {
  return (
    <div className="artikel-container">
      ...
      <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '12px', lineHeight: 1.4, letterSpacing: '-0.5px' }}>{artikel.titel}</h1>
...
function CryptoPagina({ prijzen, vorigePrijzen, valuta, historiek }: any) {
  ...
  const gefilterd = filter === 'alle' ? CRYPTO_CONFIG
    : filter === 'top' ? CRYPTO_CONFIG.slice(0, 4)
    : CRYPTO_CONFIG.filter(c => converteer(prijzen[c.id] || c.startprijs, valuta) > 100);

  return (
    <div>
      <div className="hero"><h1>₿ <span>Crypto</span></h1><p>Live cryptocurrency prijzen — klik voor grafiek</p></div>

      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
        {[['alle', 'Alle coins'], ['top', 'Top 4'], ['groot', 'Grote coins']].map(([val, label]) => (
          <button key={val} onClick={() => setFilter(val)} style={{ padding: '6px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer', background: filter === val ? 'linear-gradient(135deg, #f0b429, #e08e00)' : 'rgba(255,255,255,0.06)', color: filter === val ? '#000' : '#7d8590', fontSize: '13px', fontWeight: 600, fontFamily: 'Inter, sans-serif', transition: 'all 0.2s' }}>{label}</button>
        ))}
      </div>

      <div className="cards-grid" style={{ marginBottom: '28px' }}>
        {[
          { label: 'Totale marktcap', waarde: '€ 2.1T', kleur: '#f0b429', icoon: '📊' },
          { label: '24u volume', waarde: '€ 84.2B', kleur: '#3fb950', icoon: '📈' },
          { label: 'BTC dominantie', waarde: '52.4%', kleur: '#f7931a', icoon: '₿' },
          { label: 'Actieve coins', waarde: '8.000+', kleur: '#58a6ff', icoon: '🪙' },
        ].map(s => (
          <div key={s.label} className="stat-kaart">
            <div className="stat-label">{s.icoon} {s.label}</div>
            <div className="stat-waarde" style={{ color: s.kleur, fontSize: '20px' }}>{s.waarde}</div>
          </div>
        ))}
      </div>
      ...
      <div className="cards-grid" style={{ marginBottom: '24px' }}>
        {gefilterd.map(c => (
          <PrijsKaart key={c.id} item={c} prijs={prijzen[c.id] || c.startprijs} vorigeprijs={vorigePrijzen[c.id] || 0} valuta={valuta} onClick={() => setGeselecteerd(c)} geselecteerd={geselecteerd.id === c.id} />
        ))}
      </div>
      ...
      <div style={{ background: 'rgba(22,27,34,0.6)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '11px', fontWeight: 700, color: '#7d8590', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>
        ...
        {CRYPTO_CONFIG.map((c, i) => {
          ...
          return (
            <div key={c.id} onClick={() => setGeselecteerd(c)} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer', transition: 'background 0.2s', alignItems: 'center' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div><div style={{ fontWeight: 600, fontSize: '14px' }}>{c.naam}</div><div style={{ fontSize: '11px', color: '#7d8590' }}>#{i + 1}</div></div>
              </div>
              <div style={{ textAlign: 'right' as const, fontWeight: 600 }}>{valuta.symbool} {prijs.toLocaleString()}</div>
              <div style={{ textAlign: 'right' as const }}>
                <span style={{ color: positief ? '#3fb950' : '#f85149', fontWeight: 600, fontSize: '13px', background: positief ? 'rgba(63,185,80,0.1)' : 'rgba(248,81,73,0.1)', padding: '3px 8px', borderRadius: '6px' }}>
                  {positief ? '+' : ''}{pct}%
                </span>
              </div>
...
function LeerModulesPagina() {
      ...
      <div className="artikel-container" style={{ maxWidth: '800px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
          ...
          <div>
            ...
            <h1 style={{ fontSize: '24px', fontWeight: 700, letterSpacing: '-0.5px' }}>{geselecteerd.titel}</h1>
          </div>
        </div>
        <div className="artikel-inhoud" style={{ whiteSpace: 'pre-line' }}>{geselecteerd.inhoud}</div>
        {!voltooid.includes(geselecteerd.id) && (
          <button className="refresh-btn" style={{ marginTop: '24px' }} onClick={() => { setVoltooid(prev => [...prev, geselecteerd.id]); setGeselecteerd(null); }}>✅ Module voltooid</button>
        )}
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="hero"><h1>🎓 <span>Leer Traden</span></h1><p>Van beginner tot gevorderde trader — stap voor stap</p></div>
      <div className="cards-grid" style={{ marginBottom: '32px' }}>
        {[
          { label: 'Modules beschikbaar', waarde: `${MODULES.length}`, kleur: '#f0b429' },
          { label: 'Voltooid', waarde: `${voltooid.length}/${MODULES.length}`, kleur: '#3fb950' },
          { label: 'Voortgang', waarde: `${Math.round(voltooid.length / MODULES.length * 100)}%`, kleur: '#58a6ff' },
        ].map(s => (
          <div key={s.label} className="stat-kaart">
            <div className="stat-label">{s.label}</div>
            <div className="stat-waarde" style={{ color: s.kleur }}>{s.waarde}</div>
          </div>
        ))}
      </div>
      <div className="section-title">Alle modules</div>
          ...
          <div key={m.id} className="competitie-kaart" onClick={() => setGeselecteerd(m)} style={{ cursor: 'pointer' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: '15px', marginBottom: '3px' }}>{m.titel}</div>
                  <div style={{ fontSize: '12px', color: '#7d8590' }}>{m.duur} · {m.niveau}</div>
                </div>
              </div>
...
function PortfolioPagina({ prijzen, valuta }: any) {
  const [portfolio, setPortfolio] = useState<any[]>([]);
  const [virtueelGeld, setVirtueelGeld] = useState(10000);
  ...
  function koop(item: any) {
    ...
    setVirtueelGeld(prev => Math.round((prev - prijs) * 100) / 100);
    setPortfolio(prev => {
  ...
  const portfolioWaarde = portfolio.reduce((sum, p) => sum + converteer(prijzen[p.id] || p.startprijs, valuta) * p.aantal, 0);
  const totaal = virtueelGeld + portfolioWaarde;
  const totalePnL = totaal - 10000;

  return (
    <div>
      <div className="hero"><h1>📊 Mijn <span>Portfolio</span></h1><p>Leer traden met virtueel geld — geen echt geld nodig!</p></div>
      {boodschap && <div className="boodschap">{boodschap}</div>}

      <div className="cards-grid" style={{ marginBottom: '32px' }}>
        {[
          { label: 'Virtueel geld', waarde: `${valuta.symbool} ${virtueelGeld.toLocaleString()}`, kleur: '#3fb950' },
          { label: 'Portfolio waarde', waarde: `${valuta.symbool} ${Math.round(portfolioWaarde).toLocaleString()}`, kleur: '#f0b429' },
          { label: 'Totale P&L', waarde: `${totalePnL >= 0 ? '+' : ''}${valuta.symbool} ${Math.round(totalePnL).toLocaleString()}`, kleur: totalePnL >= 0 ? '#3fb950' : '#f85149' },
        ].map(s => (
          <div key={s.label} className="stat-kaart">
            <div className="stat-label">{s.label}</div>
            <div className="stat-waarde" style={{ color: s.kleur }}>{s.waarde}</div>
            ...
            return (
              <div key={p.id} className="positie-kaart">
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    <div style={{ color: p.kleur, fontWeight: 700 }}>{p.naam}</div>
                    <div style={{ color: '#7d8590', fontSize: '12px' }}>{p.aantal}x @ {valuta.symbool} {p.aankoopprijs}</div>
                  </div>
                </div>
                <div style={{ textAlign: 'right' as const }}>
      ...
      <div className="cards-grid">
        {alleItems.map(item => (
          <div key={item.id} className="koop-kaart">
            ...
            <div style={{ color: '#e6edf3', fontSize: '20px', fontWeight: 700, marginBottom: '14px', letterSpacing: '-0.5px' }}>
              {valuta.symbool} {converteer(prijzen[item.id] || item.startprijs, valuta).toLocaleString()}
            </div>
            <button className="koop-btn" onClick={() => koop(item)} style={{ background: `linear-gradient(135deg, ${item.kleur}, ${item.kleur}aa)`, color: '#000' }}>
              Koop 1 eenheid
            </button>
          </div>
        ))}
...
function CompetitiePagina() {
  ...
  const [competities, setCompetities] = useState([
    { id: 1, naam: 'Liga van Vrienden', deelnemers: ['Jij', 'Thomas', 'Lena', 'Sander'], startdatum: '1 mei', einddatum: '31 mei', actief: true,
      scores: [{ naam: 'Lena', waarde: 11240, winst: 12.4 }, { naam: 'Jij', waarde: 10820, winst: 8.2 }, { naam: 'Thomas', waarde: 10340, winst: 3.4 }, { naam: 'Sander', waarde: 9820, winst: -1.8 }] },
  ]);
  ...
  return (
    <div>
      <div className="hero"><h1>🏆 <span>Competitie</span></h1><p>Compete met vrienden — wie maakt de meeste winst?</p></div>
      ...
      {toonNieuw && (
        <div className="competitie-kaart" style={{ marginBottom: '20px' }}>
          ...
          <input value={nieuwNaam} onChange={e => setNieuwNaam(e.target.value)} placeholder="Naam van je competitie..." style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '12px 16px', color: '#e6edf3', fontSize: '14px', marginBottom: '12px', fontFamily: 'Inter, sans-serif', outline: 'none' }} />
      ...
      {competities.map(comp => (
        <div key={comp.id} className="competitie-kaart">
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap' as const, gap: '8px' }}>
            <div>
              <div style={{ fontSize: '17px', fontWeight: 700, marginBottom: '4px' }}>{comp.naam}</div>
              <div style={{ fontSize: '12px', color: '#7d8590' }}>{comp.startdatum} → {comp.einddatum} · {comp.deelnemers.length} deelnemers</div>
            </div>
          ...
          {[...comp.scores].sort((a, b) => b.waarde - a.waarde).map((s, i) => (
            <div key={s.naam} className="leaderboard-item">
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: s.naam === 'Jij' ? 700 : 500, color: s.naam === 'Jij' ? '#f0b429' : '#e6edf3' }}>{s.naam} {s.naam === 'Jij' && '👈'}</div>
              </div>
              <div style={{ textAlign: 'right' as const }}>
                <div style={{ fontWeight: 700 }}>€ {s.waarde.toLocaleString()}</div>
                <div style={{ fontSize: '12px', fontWeight: 600, color: s.winst >= 0 ? '#3fb950' : '#f85149' }}>{s.winst >= 0 ? '+' : ''}{s.winst}%</div>
              </div>
            </div>
          ))}
          <div style={{ marginTop: '20px', padding: '14px', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', fontSize: '13px', color: '#7d8590' }}>
            📨 Nodig vrienden uit: <span style={{ color: '#58a6ff' }}>tradelearn.app/join/{comp.id}</span>
          </div>
        </div>
      ))}
    </div>
...
function AdminPagina() {
  ...
  return (
    <div>
      <div className="hero"><h1>⚙️ <span>Admin Dashboard</span></h1><p>Beheer je platform — alleen zichtbaar voor jou</p></div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '28px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '4px', width: 'fit-content', flexWrap: 'wrap' as const }}>
        {[['overzicht', '📊 Overzicht'], ['gebruikers', '👥 Gebruikers'], ['inkomsten', '💰 Inkomsten'], ['instellingen', '⚙️ Instellingen']].map(([tab, label]) => (
          <button key={tab} onClick={() => setActieveTab(tab)} style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer', background: actieveTab === tab ? 'rgba(255,255,255,0.1)' : 'transparent', color: actieveTab === tab ? '#e6edf3' : '#7d8590', fontSize: '13px', fontWeight: 600, fontFamily: 'Inter, sans-serif', transition: 'all 0.2s' }}>{label}</button>
        ))}
      </div>
      ...
      {actieveTab === 'overzicht' && (
        <div>
          <div className="cards-grid" style={{ marginBottom: '28px' }}>
            {[
              { label: 'Totale gebruikers', waarde: '1.247', groei: '+12%', kleur: '#3fb950' },
              { label: 'Premium gebruikers', waarde: '89', groei: '+5%', kleur: '#f0b429' },
              { label: 'Actieve competities', waarde: '34', groei: '+8%', kleur: '#58a6ff' },
              { label: 'Maandelijkse omzet', waarde: '€ 445', groei: '+5%', kleur: '#9945ff' },
            ].map(s => (
              <div key={s.label} className="stat-kaart">
                <div className="stat-label">{s.label}</div>
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px' }}>
                  <div className="stat-waarde" style={{ color: s.kleur }}>{s.waarde}</div>
                  <div style={{ color: '#3fb950', fontSize: '12px', fontWeight: 600, marginBottom: '3px' }}>{s.groei}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {[
            { tekst: 'Nieuwe premium gebruiker: Thomas V.', tijd: '2 min geleden', kleur: '#f0b429' },
            { tekst: 'Nieuwe competitie: "Vrienden Liga"', tijd: '15 min geleden', kleur: '#58a6ff' },
            { tekst: 'Nieuwe gratis gebruiker: Julie P.', tijd: '1 uur geleden', kleur: '#3fb950' },
            { tekst: 'Premium betaling ontvangen: € 4,99', tijd: '2 uur geleden', kleur: '#9945ff' },
          ].map((a, i) => (
            <div key={i} className="nieuws-kaart" style={{ cursor: 'default' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: a.kleur, flexShrink: 0 }}></div>
      ...
      {actieveTab === 'gebruikers' && (
        <div>
          ...
          <div style={{ background: 'rgba(22,27,34,0.6)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', overflow: 'hidden' }}>
            ...
            {[
              { naam: 'Thomas V.', email: 'thomas@gmail.com', plan: 'Premium', datum: '1 mei', actief: true },
              { naam: 'Lena M.', email: 'lena@hotmail.com', plan: 'Premium', datum: '3 mei', actief: true },
              { naam: 'Sander K.', email: 'sander@gmail.com', plan: 'Gratis', datum: '5 mei', actief: true },
              { naam: 'Julie P.', email: 'julie@outlook.com', plan: 'Gratis', datum: '8 mei', actief: false },
            ].map((g, i) => (
      ...
      {actieveTab === 'inkomsten' && (
        <div>
          ...
          <div className="section-title">Recente betalingen</div>
          {[{ naam: 'Thomas V.', bedrag: '€ 4,99', datum: '10 mei', type: 'Maandelijks' }, { naam: 'Lena M.', bedrag: '€ 39,00', datum: '3 mei', type: 'Jaarlijks' }, { naam: 'Pieter D.', bedrag: '€ 4,99', datum: '10 mei', type: 'Maandelijks' }].map((b, i) => (
            <div key={i} className="positie-kaart">
              <div><div style={{ fontWeight: 600 }}>{b.naam}</div><div style={{ color: '#7d8590', fontSize: '12px' }}>{b.type} · {b.datum}</div></div>
              <div style={{ color: '#3fb950', fontWeight: 700, fontSize: '16px' }}>{b.bedrag}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {actieveTab === 'instellingen' && (
        <div>
          ...
          {[
            { label: 'Gratis: max virtueel geld', waarde: '€ 10.000' },
            { label: 'Premium: max virtueel geld', waarde: '€ 100.000' },
            { label: 'Gratis: max competities', waarde: '1' },
            { label: 'Premium: max competities', waarde: 'Onbeperkt' },
            { label: 'Premium per maand', waarde: '€ 4,99' },
            { label: 'Premium per jaar', waarde: '€ 39,00' },
          ].map((s, i) => (
            <div key={i} className="positie-kaart">
              <span style={{ color: '#7d8590' }}>{s.label}</span>
              <span style={{ fontWeight: 600, color: '#f0b429' }}>{s.waarde}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <BrowserRouter>
      <div className="app-bg">
        <Navbar valuta={valuta} setValuta={setValuta} />
        <div className="page">
          <Routes>
            ...
            <Route path="/portfolio" element={<PortfolioPagina prijzen={prijzen} valuta={valuta} />} />
